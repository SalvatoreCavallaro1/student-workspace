    ...
    const onSubmit = (event) => {
        ...
        if(checkForm())
        {
            ...
            createUserWithEmailAndPassword(firebase.auth,userState.email,userState.password).then(createdUser => {
                updateuserDetails(createdUser.user);  //chiamo la funzione updateuserDetails per settare il displayName e l'url della foto prodilo utilizzando l' API gravatar
            }).catch((servererror) => {
    ...
    const updateuserDetails = (createdUser) => {
        if (createdUser) {
            ...
            createdUser.user.updateProfile({
                    ...
                    photoURL: `http://gravatar.com/avatar/${createdUser.user.uid}?d=identicon`  //utilizzo l'API gravatar per creare l'avatar del profilo
